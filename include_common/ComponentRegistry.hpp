...
namespace Engine {
    using ComponentAdder = std::function<void(Engine::Entity, const void*, size_t, Engine::Mediator&)>;
    ...
    public:
        ...
        void addComponentByType(const std::string& type_name, Engine::Entity entity, const void* data, size_t size, Engine::Mediator& mediator) {
            auto it = registry_.find(type_name);
            if (it != registry_.end()) {
                it->second(entity, data, size, mediator);
            } else {
