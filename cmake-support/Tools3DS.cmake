...
if (NOT 3DS)
    message (
        WARNING
        "Those tools can only be used if you are using the 3DS toolchain file. Please erase this build directory or create another one, and then use -DCMAKE_TOOLCHAIN_FILE=DevkitArm3DS.cmake when calling cmake for the 1st time. For more information, see the Readme.md for more information."
    )
endif ()

# Used to locate files to be used with configure_file
get_filename_component (
    __tools3dsdir ${CMAKE_CURRENT_LIST_FILE} PATH
)
...
set (
    FORCE_SMDHTOOL FALSE
    CACHE BOOL "Force the use of smdhtool instead of bannertool"
)

#############
## MAKEROM ##
#############
# Not part of devkitArm anymore? Need to be reworked.
if( NOT MAKEROM)
...
if (NOT PICASSO_EXE)
    ...
    if (PICASSO_EXE)
        ...
        set (
            SHADER_AS picasso
            CACHE STRING "The shader assembler to be used. Allowed values are 'none', 'picasso' or 'nihstro'"
        )
    else ()
...
function (add_3dsx_target target)
    ...
    if ((NOT (${ARGC} GREATER 1 AND "${ARGV1}" STREQUAL "NO_SMDH") ) OR (${ARGC} GREATER 3) )
        ...
        add_custom_command (
            OUTPUT ${CMAKE_CURRENT_BINARY_DIR}/${target_we}.3dsx
            COMMAND ${_3DSXTOOL} $<TARGET_FILE:${target}> ${CMAKE_CURRENT_BINARY_DIR}/${target_we}.3dsx --smdh=${CMAKE_CURRENT_BINARY_DIR}/${target_we}.smdh --romfs=${CMAKE_BINARY_DIR}/romfs #TEMP FOR ROMFS
...
function (add_cia_target target RSF IMAGE SOUND )
    ...
    add_custom_command (
        OUTPUT ${CMAKE_CURRENT_BINARY_DIR}/${target_we}.cia
...
macro (add_binary_library libtarget)
    ...
    if (NOT ENABLED_LANGUAGES MATCHES ".*ASM.*")
        message (
            FATAL_ERROR
            "You have to enable ASM in order to use add_binary_library (or any target_embed_* which relies on it). Use enable_language(ASM) in your CMakeLists. Currently enabled languages are ${ENABLED_LANGUAGES}"
        )
    endif ()
    foreach (__file ${ARGN})
        get_filename_component (__file_wd ${__file} NAME)
        # Add '_' if the file name starts by a number.
        string (REGEX REPLACE "^([0-9])" "_\\1" __BIN_FILE_NAME ${__file_wd})
    ...
    add_custom_command (
        OUTPUT ${CMAKE_CURRENT_BINARY_DIR}/binaries_asm/${libtarget}.s
...
macro (target_texture OUTPUT_D OUTPUT_T3X INPUT)
    if (${ARGC} LESS 3)
        message (FATAL_ERROR "target_texture: Output and input files missing")
    endif ()
    if (${ARGC} GREATER 3)
        message (FATAL_ERROR "target_texture: Too many arguments")
    endif ()
    add_custom_command (
        OUTPUT ${OUTPUT_D} ${OUTPUT_T3X}
        COMMAND ${TEX3DS} -i ${INPUT} -d ${OUTPUT_D} -o ${OUTPUT_T3X}
        DEPENDS ${INPUT}
    )
endmacro ()

macro (target_bin2o OUTPUT_O OUTPUT_H INPUT)
    if (${ARGC} LESS 3)
        message (FATAL_ERROR "target_bin2o: Output and input files missing")
    endif ()
    if (${ARGC} GREATER 3)
        message (FATAL_ERROR "target_bin2o: Too many arguments")
    endif ()
    add_custom_command (
        OUTPUT ${OUTPUT_H} ${OUTPUT_O}
        COMMAND bin2s -a 4 -H ${OUTPUT_H} ${INPUT} | ${DEVKITARM}/bin/arm-none-eabi-as -o ${OUTPUT_O}
        #WORKING_DIRECTORY ${CMAKE_CURRENT_LIST_DIR}
        DEPENDS ${INPUT}
    )
endmacro ()
...
macro (add_shbin OUTPUT INPUT )
    ...
    elseif (SHADER_AS STREQUAL "nihstro")
        ...
        add_custom_command (
            OUTPUT ${OUTPUT}
            COMMAND ${NIHSTRO_AS} ${INPUT} -o ${OUTPUT} -e ${_ENTRYPOINT} ${SHADER_TYPE_FLAG}
            WORKING_DIRECTORY ${CMAKE_CURRENT_LIST_DIR}
        )
    else ()
